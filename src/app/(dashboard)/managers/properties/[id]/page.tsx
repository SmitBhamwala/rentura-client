...
export default function PropertyTenants() {
  ...
  const { data: leases, isLoading: leasesLoading } = useGetPropertyLeasesQuery(
    propertyId,
    {
      skip:
        !authUser?.cognitoInfo?.userId ||
        !managerProperties?.some((p) => p.id === propertyId)
    }
  );
  ...
  if (!authUser || !managerProperties?.some((p) => p.id === propertyId)) {
    return (
      <div className="text-center mt-8">
        You do not have permission to view this property or it does not exist.
      </div>
    );
  }

  if (!property || propertyError) {
    return <div className="text-center">Failed to load property</div>;
  }
  ...
  return (
    <div className="dashboard-container">
      ...
      <Link
        ...
        className="flex items-center mb-4 underline underline-offset-2 hover:text-primary-500"
        scroll={false}>
